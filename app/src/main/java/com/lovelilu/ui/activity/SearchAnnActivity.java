 ...
 */
public class SearchAnnActivity extends SwipeBackActivity implements BGARefreshLayout.BGARefreshLayoutDelegate {
    ...
    private void init() {
        ...
        String json = getIntent().getStringExtra(Constants.KEY.JSON_LIST_ANN);
        mList = JSONUtil.fromJson(json, new TypeToken<List<Anniversary>>() {
        }.getType());
    }
...
//        rl_refresh.setBackgroundColor(getResources().getColor(R.color.bg));
    ...
    private void initAdapter() {
        mAdapter = new AnnListAdapter(this, R.layout.item_ann_qo, mList);
...
//        mainQuery.or(queries);
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                final List<Anniversary> result = new ArrayList<>();
                for (Anniversary a : mList) {

                    if (a.getType() == Constants.ANN_TYPE.TYPE_DAY) {
                        if (a.getTitle().contains(keyWord)) {
                            result.add(a);
                        }
                    }

                    if (a.getType() == Constants.ANN_TYPE.TYPE_QUOTATION) {
                        if (a.getQuotation().contains(keyWord)) {
                            result.add(a);
                        }
                    }
    ...
    private void showListData(List<Anniversary> list) {
